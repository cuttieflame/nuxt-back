<template>
  <div>
    <template v-if="this.$route.query.email_verify_url">
      <button @click="verify(verify_email)" class="btn btn-primary">Подтвердить почту</button>
    </template>
    <template>
    <hr class="mt-0 mb-4">
    <div class="row">
          ...
          <div class="card-body">
            <form method="POST" @submit.prevent="changePassword()">
              ...
              <div class="mb-3">
                ...
                <input class="form-control" id="currentPassword" type="password" placeholder="Enter current password" v-model=old_password>
              </div>
              <!-- Form Group (new password)-->
              <div class="mb-3">
                <label class="small mb-1" for="newPassword">New Password</label>
                <input class="form-control" id="newPassword" type="password" placeholder="Enter new password" v-model=new_password>
              </div>
              <!-- Form Group (confirm password)-->
              <div class="mb-3">
                <label class="small mb-1" for="confirmPassword">Confirm Password</label>
                <input class="form-control" id="confirmPassword" type="password" placeholder="Confirm new password" v-model=confirm_password>
              </div>
      ...
      <div class="col-lg-4">
        ...
        <!-- Delete account card-->
        <div class="card mb-4">
          <div class="card-header">Подтвердить почту</div>
          <div class="card-body">
            <div v-if='typeof $auth.user.data.email_verified_at !== "string"'>
            <p>Чтоб подтвердить почту мы вышлем на вашу почту ссылку</p>
            <button @click="verifyEmail()" class="btn btn-outline-danger" type="button">Отправить письмо</button>
            </div>
            <div v-else>
              <p>Ваша почта подтверждена,но вы также можете её сбросить</p>
              <button class="btn btn-outline-danger">Сбросить почту</button>
            </div>
          </div>
        </div>
        <div class="card mb-4">
          ...
          <div class="card-body">
            ...
            <button class="btn btn-outline-danger" type="button">I understand, delete my account</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    if(this.$route.query.email_verify_url) {
      let a = this.$route.query.email_verify_url.indexOf('api/v1')
      this.verify_email = this.$route.query.email_verify_url.substr(a)
    }
  },
  methods: {
    verifyEmail() {
      this.$axios.$get('/api/v1/email/resend')
    },
    verify(id) {
      this.$axios.$get(`${id}`)
    },
    changePassword() {
        this.$axios.$post('/api/v1/password/password-reset',{
